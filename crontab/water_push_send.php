...
$alarm_results = (new Database('alarm'))->execute("
                select * from water_alarm as a 
                    left join widget as w
                    on a.device_idx = w.device_idx
                where a.member_idx = ".$member_idx." and a.activation = 'Y'
                 ");
...
while ($alarm_obj = $alarm_results->fetchObject(EntityAlarm::class)) {
    $raw_check = Common::water_alarm_validity_check($alarm_obj);
        if ($raw_check['reslut']) {
...
if (!empty($alarm_array)) {
    ...
    Common::wateralarmHistoryInsert($member_idx, $alarm_contents, $alarm_history_array);
